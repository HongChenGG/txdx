...
function patchNavigator(win) {
  ...
  // Chrome exposes webdriver on Navigator.prototype, not as an own property.
  // TDC explicitly checks Object.getOwnPropertyDescriptor(navigator, "webdriver").
...
function patchTouchSurface(win) {
  // The captured Chrome profile has maxTouchPoints but does not expose
  // ontouchstart on a freshly-created div. jsdom adds it to HTMLElement.
  for (const ctor of [win.HTMLElement, win.SVGElement]) {
    try { delete ctor.prototype.ontouchstart; } catch (_) {}
  }
}
...
function make2dContext(canvas) {
  // toDataURL() above returns the captured browser fingerprint, so the
  // production package only needs this method-compatible drawing stub.
  const noop = () => {};
  return {
    canvas,
    fillStyle: "#000", strokeStyle: "#000", lineWidth: 1,
    globalAlpha: 1, globalCompositeOperation: "source-over",
    textBaseline: "alphabetic", textAlign: "start", font: "10px sans-serif",
    ...
    bezierCurveTo: noop, quadraticCurveTo: noop, stroke: noop, fill: noop,
    arc: noop, arcTo: noop, rect: noop, save: noop, restore: noop,
...
function makeWebGLContext() {
  ...
  const MAX_RENDERBUFFER_SIZE = 0x84fd;
  ...
  return {
    getParameter: (param) => {
      switch (param) {
        ...
        case MAX_VIEWPORT_DIMS: return new Int32Array([32767, 32767]);
...
function patchComputedStyle(win) {
  if (typeof win.getComputedStyle !== "function") return;
  const original = win.getComputedStyle.bind(win);
  win.getComputedStyle = function getComputedStyle(element, pseudoElement) {
    ...
    style.getPropertyValue = function (property) {
      ...
      // Chrome resolves the CSS-wide `revert` keyword to the UA value.
      // jsdom 25 leaks the literal keyword for color instead.
      return property === "color" && value === "revert" ? "rgb(0, 0, 0)" : value;
    };
    return style;
  };
}

function patchMediaQueries(win) {
  ...
  win.matchMedia = function matchMedia(query) {
    ...
    const matches =
      /\(prefers-color-scheme:\s*light\)/.test(normalized) ||
      /\(prefers-reduced-motion:\s*no-preference\)/.test(normalized) ||
      /\(min-width:\s*(?:[0-9]{1,3})px\)/.test(normalized);
    return {
      matches,
      media,
      onchange: null,
...
function patchMediaCapture(win) {
  const proto = win.HTMLMediaElement && win.HTMLMediaElement.prototype;
  if (!proto || "captureStream" in proto) return;
  try {
...
function patchRTC(win) {
  // tdc.js probes window.RTCPeerConnection (fingerprint via createDataChannel /
  // localDescription stats). jsdom has none. Provide a Chrome-like stub so the
  // probe yields data instead of undefined (a hard bot signal).
  ...
  class RTCPeerConnection {
    constructor(cfg) { this._cfg = cfg || {}; this.localDescription = null; this.remoteDescription = null; this.iceGatheringState = "new"; this.iceConnectionState = "new"; this.connectionState = "new"; this.signalingState = "stable"; this.onicecandidate = null; this.ontrack = null; this.onconnectionstatechange = null; this.oniceconnectionstatechange = null; }
    createOffer() { const o = { type: "offer", sdp: "v=0\r\no=- 0 0 IN IP4 0.0.0.0\r\ns=-\r\nt=0 0\r\na=group:BUNDLE 0\r\na=msid-semantic: WMS\r\nm=video 9 UDP/TLS/RTP/SAVPF 96\r\na=rtpmap:96 VP8/90000\r\n" }; this.localDescription = o; return Promise.resolve(o); }
    createAnswer() { const a = { type: "answer", sdp: "v=0\r\no=- 1 1 IN IP4 0.0.0.0\r\ns=-\r\nt=0 0\r\nm=audio 9 UDP/TLS/RTP/SAVPF 111\r\na=rtpmap:111 opus/48000/2\r\n" }; this.localDescription = a; return Promise.resolve(a); }
    setLocalDescription(desc) { this.localDescription = desc || { type: "offer", sdp: "" }; return Promise.resolve(); }
    setRemoteDescription(desc) { this.remoteDescription = desc || { type: "answer", sdp: "" }; return Promise.resolve(); }
    addIceCandidate() { return Promise.resolve(); }
    createDataChannel(label, opts) { return { label: label || "", ordered: true, protocol: "", readyState: "connecting", send: noop, close: noop, onopen: null, onclose: null, onmessage: null, onerror: null }; }
    addTrack() { return { track: { kind: "video", id: "track-1", enabled: true }, transceiver: null }; }
    getTransceivers() { return []; }
    getSenders() { return []; }
    getReceivers() { return []; }
    getStats() { return Promise.resolve(new Map()); }
    addTransceiver() { return { direction: "sendrecv", sender: null, receiver: null, mid: null, setDirection: noop }; }
    close() {}
  }
  win.RTCPeerConnection = RTCPeerConnection;
  win.RTCIceCandidate = RTCIceCandidate;
  if (win.RTCSessionDescription === undefined) {
...
function apply(win) {
  ...
  patchCanvas(win);
  patchComputedStyle(win);
  patchMediaQueries(win);
  patchMediaCapture(win);
  patchAudio(win);
  patchFonts(win);
